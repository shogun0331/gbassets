...
namespace GB
{
    ...
    public class AssetData 
    {
            ...
            if(DependencyList != null && DependencyList.Count > 0)
            {
                for(int i = 0; i< DependencyList.Count; ++i)
                {
                    if( DependencyList[i].PackageType  == AssetModel.PACKAGE_TYPE.GIT)
                        DownloadURLPackage(DependencyList[i].URL);
                    else 
                        DownloadUnityPackage(DependencyList[i].URL,DependencyList[i].Key);
                }
            }

            if(Model.PackageType == AssetModel.PACKAGE_TYPE.GIT)
                DownloadURLPackage(Model.URL);
            else
                DownloadUnityPackage(Model.URL,Model.Key);
        }




        private void DownloadURLPackage(string url)
        {
        ...
        private void DownloadUnityPackage(string url, string fileName)
        {
